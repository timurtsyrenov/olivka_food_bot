    ...
    def parse(self) -> LunchMenu:
        ...
        try:
            ...
            if response.content is not None:
                ...
                lunch_items: list = [
                    item.strip().replace(black_text[0], "").strip()
                    for item in dishes
                    if item.strip() and black_text[1] not in item.strip()
                ]
                ...
                return LunchMenu(lunch_name=lunch_name, lunch_price=lunch_price, lunch_items=lunch_items, day=self.day)
            else:
